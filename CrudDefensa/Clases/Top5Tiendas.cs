...
{
    public partial class Top5Tiendas : Form
    {
        ...
        private void cargartop5tiendas()
        {
            // Cadena de conexión
            ...
            using (NpgsqlConnection conexion = new NpgsqlConnection(cadenaConexion))
            {
                try
                {
                    ...
                    // Consulta SQL
                    string consulta = "SELECT * FROM top_5_tiendas_ventas_anio ";
        ...
        private void bd5tiendas()
        {
            using (var connection = new NpgsqlConnection(cadenaConexion))
            {
                connection.Open();

                using (var cmd = new NpgsqlCommand("SELECT * FROM top_5_tiendas_ventas_anio ", connection))
                {
