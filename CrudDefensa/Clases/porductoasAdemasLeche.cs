...
{
    public partial class porductoasAdemasLeche : Form
    {
        ...
        private void cargarinventario()
        {
            ...
            using (NpgsqlConnection conexion = new NpgsqlConnection(cadenaConexion))
            {
                try
                {
                    ...
                    // Consulta SQL
                    string consulta = "SELECT * FROM top_3_productos_ademas_leche ";
        ...
        private void bdproductosLeche()
        {
            using (var connection = new NpgsqlConnection(cadenaConexion))
            {
                connection.Open();

                using (var cmd = new NpgsqlCommand("SELECT * FROM top_3_productos_ademas_leche ", connection))
                {
